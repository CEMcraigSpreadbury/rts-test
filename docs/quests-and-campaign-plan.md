...
- 2026-09-16: **Embers on the Border, mission 4: The Siege of Aldmere**
  (`scenes/scenarios/e04_the_siege_of_aldmere.tscn`,
  `resources/scenarios/the_siege_of_aldmere.tres`) on a **new map, Aldmere
  Vale** — 2 players, size 176, a 14-unit clearing round each base with its
  gold at 15-18 and trees at 20-28 (the default gold distance sits inside a
  clearing that wide, and the first generation had no base gold at all),
  Beastmen points at the centre and flanks, no shrines, seed 1212. Settings
  saved as `scenes/tools/map_generator_aldmere_vale.tscn`.
  The capital (Player slot, south-west) starts with a **palisade ring** placed
  in the scene: 38 wall segments and 2 gates at radius 13 (gates facing the
  west gold and the road to the centre), 3 watchtowers, a Barracks, a
  Blacksmith and two Houses. Walls are ordinary independent buildings, so a
  ring is just pieces with a rotation each — generated by a script (each
  segment's local X along the tangent, `rotation.y = -(θ + 90°)`). Placed
  buildings are adopted before the first navmesh carve, so the ring blocks
  paths from the start: the camps' routes to the capital come in longer than
  the straight line.
  Steps: hold 10 minutes while five hidden timer steps send waves from a
  north and an east camp (a DEFENDERS side with two Beastmen Barracks and
  guards each); the fourth wave brings a **Skeleton Dragon** spawned with
  `name_as = "SiegeDragon"`. The hold's completion brings 6 Cavalry and 4
  Spearmen, reveals and marks both camps, and switches the Neighbour to
  Attack at the north camp. Then burn each camp and slay the dragon — three
  steps, and a hidden step requiring all three wins.
  **Engine changes:**
  - `SpawnUnitsAction.name_as` — names what it spawns (numbered when more than
    one) in `Main.scenario_entities`, so a step can target a unit that only
    arrives mid-mission (DestroyTargets / TransferOwnership).
  - **Scenario defeat was broken for every garrison-enemy mission.**
    `_check_for_game_over` returned early whenever only one team held main
    bases — always true when the enemy is DEFENDERS (Emberford, the Druids'
    Grove, this siege), so a team that lost every base kept playing and the
    loss was never recorded. In a scenario it now ends the mission as lost
    once every player seat holding a main base is out, and **never** hands out
    an elimination victory: winning is the quest's (so its closing lines
    play). Consequence for The Beacon Road: razing the warlord's Town Center
    no longer wins by itself; its optional "raze the camp" step still does.
  Verified headless: 38 walls + 2 gates + 3 towers + 4 buildings adopted for
  the player, 4 pens for the horde; both camp routes reach the capital's
  interior; the waves grew the horde 6 → 33 with the dragon registered at the
  fourth; relief spawned 6 Cavalry; burning both camps and the dragon won and
  recorded; destroying both lords' Town Centers ended in Defeat.

- 2026-09-16: **Embers on the Border, mission 3: The Druids' Grove**
  (`scenes/scenarios/e03_the_druids_grove.tscn`,
  `resources/scenarios/the_druids_grove.tres`) on a **new map, Greywood**
  (owner's call). Greywood was generated headless with MapGenerator — 2
  players, size 160, roads to the centre, a shrine at the centre and one near
  each base, two plain capture points, `forest_clusters_per_player` 12,
  `tree_spacing` 1.7, seed 4242 — and, unlike Four Kingdoms, its settings are
  saved: **`scenes/tools/map_generator_greywood.tscn`** is a generator scene
  that reproduces it (open, tweak, Save As Map; Overwrite Existing is set).
  Mission: the Mystic's envoy **Isolde** (a placed villager under the Player
  slot, `display_name` / `max_health` overridden on the instance) must be
  walked to the grove's edge (an ambush springs from two markers), the
  **druids' ring** (a DEFENDERS side) broken, and the centre shrine — point
  **E** — taken with Isolde standing in its ring. The Scenario's own modifiers
  set `allowed_monsters = ["Phoenix"]`, so every shrine on the map is guarded
  by a maddened Phoenix and offers only the Phoenix; taking the cleansed
  shrine and training one wins. "Keep Envoy Isolde alive" runs throughout and
  loses the mission if she dies. A hidden timer sends a second ambush from the
  west 45s after the ring breaks. The Neighbour seat is in Attack at `Grove`.
  **Placement trap avoided:** a shrine's neutral guard reaches about 18 units
  (leash 12.5 + aggro 6) and fights every side, DEFENDERS included — druids
  ringed round the shrine would have been killed by the Phoenix within seconds
  and completed "break the ring" by themselves. The ring sits 28 units out,
  on the road in.
  Verified headless: centre is E offering only Phoenix with one guard, a
  34-point navmesh path from Isolde to the grove, every marker on the navmesh,
  the full chain to Victory with both steps' dialogue and the first ambush
  spawning, win recorded, and Isolde's death ending in Defeat.
